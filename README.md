...
- ASCII Downzone
- Smart Internet Connection Keep-Alive
...
4. **Internet Connection Monitoring & Keep-Alive:**  
   The script periodically checks internet connectivity using ultra-lightweight network packets.
   - Prevents shared/mobile connections from being dropped due to inactivity
   - Automatically pauses download start until the connection is restored
   - Avoids terminal spam by logging connection changes only when state changes
5. **Start & End Time:** Set the download window dynamically; the script will wait until the start time and stop automatically at the end time.  
6. **Links File:** Provide a text file (`links.txt`) containing all URLs to download. The script validates the file path interactively.  
7. **Resume & Retry:** Downloads are resumable and will retry up to 3 times if a failure occurs.  
8. **Progress & Messages:** Download progress is shown in the terminal along with colored status messages.  
9. **Safe Execution:** The script will **never stop** due to individual download failures and will continue to the next link.  
10. **ASCII Banner:** Displays a large DOWNZONE banner with license and copyright information at startup.
...
Downzone automatically checks and installs required dependencies during installation.

Required tools:
- `curl` (for downloading files)
- `ping` (for internet connectivity monitoring)

On macOS:
- Uses Homebrew (must be installed beforehand)

On Linux:
- Uses the system package manager (apt, dnf, yum)
...
During the waiting period, Downzone actively keeps the internet connection alive.
If the connection is lost:
- A warning message is shown once
- The script waits silently until the connection is restored
- Downloads will NOT start until internet access is available
...
- Required dependencies are automatically checked and installed during setup.
...
Commercial use of this software requires a separate license. Contact the copyright holder at `miladpersonal1999@gmail.com` for commercial licensing.

## Changelog
### v1.1.0
- Added smart internet connection keep-alive
- Silent and state-aware connectivity logging
- Robust dependency checking and installation
- Improved installer safety (no Homebrew sudo usage)
